...
func TestServiceCreateFailsClosedWhenVoiceAdmissionIsDisabled(t *testing.T) {
	...
	svc := NewService(store, &stopRecorder{}, cfg, ServiceOptions{})
...
func TestServiceCreateUsesHashedCognitoSubjectForInternalRollout(t *testing.T) {
	...
	allowed := NewService(newMemoryStore(), &stopRecorder{}, cfg, ServiceOptions{})
	...
	store := newMemoryStore()
	denied := NewService(store, &stopRecorder{}, cfg, ServiceOptions{})
...
func TestInternalRolloutHashesTheCanonicalPhonePoolIdentity(t *testing.T) {
	...
	if _, err := NewService(newMemoryStore(), &stopRecorder{}, cfg, ServiceOptions{}).Create(
