...
func TestInvoiceAndDocumentRoutesRequireDocumentPermissions(t *testing.T) {
	...
	requiredFragments := []string{
		...
		`invoices.POST("/:id/send", middleware.RequirePermission(svcs.BusinessAuth, services.PermissionDocumentsManage), wafUserWriteRL, h.Invoice.Send)`,
	...
	requiredByFile := map[string]struct {
		...
		"lambda": {
			...
			fragments: []string{
				...
				`CREDENTIAL_ENCRYPTION_KEY_SSM_PARAM`,
				`LLM_API_KEY_SSM_PARAM`,
				`EXA_API_KEY_SSM_PARAM`,
				`GST_LOOKUP_API_KEY_SSM_PARAM`,
				`DEEPGRAM_API_KEY_SSM_PARAM`,
				`DEEPSEEK_API_KEY_SSM_PARAM`,
			},
		},
	}
	for name, check := range requiredByFile {
